...
def fd_detector(method = 'SIFT', lowe_ratio = 0.6, good_match_thresh = 0):
    ...
    for type_of_class in os.listdir('./Processed images/'):
        process_img_path = os.path.join(process_img_path, type_of_class)
        for img_path in os.listdir(process_img_path):
            full_img_path = os.path.join(process_img_path, img_path)
            ...
            # data
            with open('./all_classes_cv/annotations.xml', 'r') as f:
                data = f.read()
